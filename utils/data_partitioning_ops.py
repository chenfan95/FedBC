...
class PartitionDataset(object):
	...
	def __init__(self, input, output, partition_classes=None, partition_classes_stats=None):
		self.input = input
		self.output = output
		self.partition_size = np.size(self.output)
		self.partition_classes = partition_classes
		self.partition_classes_stats = partition_classes_stats

		if partition_classes is None:
			self.compute_partition_classes()
		if partition_classes_stats is None:
			self.compute_partition_distribution()

	def compute_partition_classes(self):
	...
	def compute_partition_distribution(self):
		...
		for key in data_counter.keys():
			partition_class_distribution[key] = \
				PartitionDatasetClassStats(class_id=key,
